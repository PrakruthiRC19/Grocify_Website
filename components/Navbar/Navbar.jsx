...
const Navbar = () => {
    ...
    useEffect(()=>{
        const handleScroll=()=>{
            setIsSrolled(window.scrollY > 10)
        }
    ...
    return (
        <header className={`bg-white fixed top-0 right-0 left-0 z-50 ${isScrolled?'drop-shadow-[0_4px_25px_rgba(0,0,0,0.1)]':""}`}>
            <nav className="max-w-[1400px] mx-auto px-10 md:h-[14vh] h-[12vh] flex justify-between items-center">
                ...
                <ul className={`flex flex-col gap-y-12 bg-orange-500/15 backdrop-blur-xl rounded-xl p-10 items-center shadow-xl md:hidden absolute top-30 left-1/2 -translate-x-1/2 transition-all duration-500 ${showMenu ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-5 pointer-events-none"}`}>
                    <li>
                        <a href="" className="font-semibold tracking-wider text-orange-500">Home</a>
                    </li>
                    <li>
